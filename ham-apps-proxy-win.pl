...

{
    ...
    # Uncomment Win32::OLE library if using under Windows with OmniRig
    use Win32::OLE;
...
=cut
    ...
    sub resp_rigctl {
        ...
        $parameters = join '', map { VALUETAG( $_, $p{$_} ) } @k;

        if ( $command eq "f" ) {
            my $freq = $response / 1000000;
            ...
            warn stamp, "VFO Freq: $freq", " VFO Mode: $mode", " VFO Width: $width";
            ...
            print qq!{"frequency":"$freq","mode":"$mode","width":"$width"}!;
        }
        ...
        if ( $command eq "M" && $parameters gt 0 ) {
            ...
            warn stamp, "RIGCTL Command: $command Value: $parameters Mode: $mode ModeCode: $mode Bandwidth: $bw";
